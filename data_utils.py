...
def crop(img1, img2, mask, size, ignore_value=255):
    w, h = img1.size
    padw = size - w if w < size else 0 # 计算需要padding的大小，保证至少有size * size
    ...
    y = random.randint(0, h - size) # 这里生成随机裁剪的左上角坐标 如果图像本身比size大，就会随机裁剪；416 *416的就裁乘406 * 406的
...
def get_transform(convert=True, normalize=False, norm_type='map'):
    transform_list = []
    if convert:
        transform_list += [transforms.ToTensor()]
    if normalize and norm_type == 'map':
        transform_list += [transforms.Normalize((0.5, 0.5, 0.5),
                                                (0.5, 0.5, 0.5))]
    elif normalize and norm_type == 'ImageNet':
...
class TestDatasetFromFolder(Dataset):
    def __init__(self, args, Time1_dir, Time2_dir, Label_dir, norm_type='map',):
        ...
        self.transform = get_transform(convert=True, normalize=True, norm_type=norm_type)  # convert to tensor and normalize to [-1,1]
    ...
    def __getitem__(self, index):
        ...
        # label = make_one_hot(label.unsqueeze(0).long(), self.n_class).squeeze(0)
...
class PredictDatasetFromFolder(Dataset):
    def __init__(self,args,  Time1_dir, Time2_dir):
        ...
        self.transform = get_transform(convert=True, normalize=False)  # convert to tensor and normalize to [-1,1] # 这里normalize=False 所以不用管norm_type
        self.label_transform = get_transform()
...
class DA_DatasetFromFolder(Dataset):
    def __init__(self, Image_dir1, Image_dir2, Label_dir, crop=True,img_size=504, augment = True, angle = 30, norm_type='map'):
        ...
        self.img_transform = get_transform(convert=True, normalize=True, norm_type=norm_type)
    ...
    def __getitem__(self, index):
        ...
        # 如果图像大小不对的话是需要裁一下的
        image1, image2, label = crop(image1, image2, label, size = img_size)
...
class TrainDatasetFromFolder(Dataset): # validation用这个，不用crop，不用augment，参考uniMatch v2的写法
    def __init__(self, opt, Image_dir1, Image_dir2, Label_dir, Txt_dir,norm_type='map'):
    ...
    def __getitem__(self, index):
        ...
        if self.n_class == 2:
            ...
            label = torch.tensor(np.array(label) / 1.0)
            # label = make_one_hot(label.unsqueeze(0).unsqueeze(0).long(), 5).squeeze(0) # HRSCD
            # label = make_one_hot(label.unsqueeze(0).unsqueeze(0).long(), self.n_class).squeeze(0)
...
class DA_DatasetFromFolder(Dataset):
    def __init__(self, opt, Image_dir1, Image_dir2, Label_dir, Txt_dir, crop=True, augment = True, angle = 30, norm_type='map', img_size=504):
        super(DA_DatasetFromFolder, self).__init__()
        # 获取图片列表
        datalist = getDataList(Txt_dir)
        self.n_class = opt.n_class
    ...
    def __getitem__(self, index):
        ...
        image1, image2, label = self.data_augment(image1, image2,label)
        image1, image2, label = crop(image1, image2, label, size = self.img_size)
...
class WODA_DatasetFromFolder(Dataset):
    def __init__(self, opt, Image_dir1, Image_dir2, Label_dir, Txt_dir, crop=True, augment = True, angle = 30, norm_type='map', img_size=504):
        super( WODA_DatasetFromFolder, self).__init__()
        # 获取图片列表
        datalist = getDataList(Txt_dir)
        self.n_class = opt.n_class
        self.in_chan = opt.in_chan
...
class TestDatasetFromFolder(Dataset):
    def __init__(self, opt, Image_dir1, Image_dir2, Label_dir, Txt_dir,norm_type):
        super(TestDatasetFromFolder, self).__init__()
        # 获取图片列表
        datalist = getDataList(Txt_dir)
        self.n_class = opt.n_class

...
def get_seg_metrics(total_correct,total_label,total_inter,total_union,total_FPR,total_pred,num_classes):
    ...
    inter_scd = np.sum(total_inter[1:])          # 变化类的正确预测像素数
    union_scd = np.sum(total_union[1:])          # 变化类的预测 + GT 总像素数（用于 precision）
    pred_scd  = np.sum(total_pred[1:])           # 变化类的预测总像素数（用于 recall）

    P_scd = inter_scd / (union_scd + np.spacing(1))
    R_scd = inter_scd / (pred_scd  + np.spacing(1))
    Fscd  = (2 * P_scd * R_scd) / (P_scd + R_scd + np.spacing(1))
...
def eval_metrics_direct(predict, target, num_class):
    predict = predict + 1                     # 每个都加1避免从0开始
    target = target + 1
    # 假设 predict 和 target 都是已经处理好的类别标签
    labeled = (target > 0) * (target <= num_class)   # 创建一个蒙版
    correct, num_labeled = batch_pix_accuracy(predict, target, labeled)   # 计算正确的像素点和总像素点
    inter, union, area_FPR, area_pred = batch_intersection_union(predict, target, num_class, labeled)
    return [np.round(correct, 5), np.round(num_labeled, 5), np.round(inter, 5), np.round(union, 5), np.round(area_FPR, 5), np.round(area_pred, 5)]
